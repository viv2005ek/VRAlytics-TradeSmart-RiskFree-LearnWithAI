...
import { Eye, EyeOff, Mail, Lock, TrendingUp, Loader2, CheckCircle, Sparkles, Zap, Star, Hexagon } from 'lucide-react';
...
export const Signup: React.FC<SignupProps> = ({ onToggleMode }) => {
  ...
  const [liquidFields, setLiquidFields] = useState<{ email?: boolean; password?: boolean; confirmPassword?: boolean }>({});
  ...
  const triggerValidationEffects = (fieldErrors: { 
    email?: string; 
    password?: string; 
    confirmPassword?: string; 
    general?: string;
    ...
    const newLiquidFields: { email?: boolean; password?: boolean; confirmPassword?: boolean } = {};
      ...
      >
          ...
          <div className="max-w-md w-full">
            <div className="backdrop-blur-xl bg-black/30 border border-emerald-400/20 rounded-3xl shadow-2xl p-8 text-center transform animate-bounce-in">
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-emerald-600 to-green-600 text-white py-4 px-6 rounded-xl font-medium hover:from-emerald-700 hover:to-green-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all duration-300 transform hover:scale-105"
              >
                Back to Sign In
              </button>
            </div>
        ...
        <div className="fixed bottom-4 right-4 z-50">
          <a href="https://bolt.new/?rid=os72mi" target="_blank" rel="noopener noreferrer" className="block transition-all duration-300 hover:shadow-2xl">
            <img src="https://storage.bolt.army/logotext_poweredby_360w.png" alt="Powered by Bolt.new badge" className="h-8 md:h-10 w-auto shadow-lg opacity-90 hover:opacity-100 bolt-badge bolt-badge-intro" onAnimationEnd={(e) => e.currentTarget.classList.add('animated')} />
          </a>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute top-20 left-20 w-72 h-72 bg-violet-500/15 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-fuchsia-500/15 rounded-full blur-3xl animate-pulse delay-1000"></div>
        <div className="absolute top-1/2 left-1/2 w-64 h-64 bg-pink-500/15 rounded-full blur-3xl animate-pulse delay-2000"></div>
        <div className="absolute top-40 right-40 w-48 h-48 bg-cyan-500/10 rounded-full blur-3xl animate-pulse delay-500"></div>
        ...
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(12)].map((_, i) => {
            const icons = [Sparkles, Zap, Star, Hexagon];
            ...
            return (
              <div
                ...
                style={{
                  ...
                  transform: `translateZ(${i * 8}px) rotateY(${mousePosition.x * 0.1}deg) rotateX(${mousePosition.y * 0.1}deg)`,
                  transition: 'transform 0.2s ease-out'
                }}
              >
                <IconComponent 
                  className={`w-5 h-5 text-violet-400/30 transform rotate-${i * 25}`}
                  style={{
        ...
        {/* Enhanced Mouse Follower with Gradient */}
        <div
          className="absolute w-96 h-96 bg-gradient-radial from-violet-500/5 to-transparent rounded-full pointer-events-none transition-all duration-300 ease-out"
          style={{
        ...
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(20)].map((_, i) => (
            <div
              key={i}
              className="absolute w-1 h-1 bg-violet-400/20 rounded-full animate-particle"
              style={{
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md w-full">
          {/* Glassmorphism Card - Same interactivity as Login */}
          <div 
            className={`backdrop-blur-xl bg-black/20 border border-violet-400/20 rounded-3xl shadow-2xl p-8 transform transition-all duration-300 hover:scale-105 hover:bg-black/30 ${
          ...
          >
            ...
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 backdrop-blur-sm rounded-2xl mb-6 transform transition-all duration-300 hover:scale-110 hover:rotate-12 border border-violet-400/10">
                <TrendingUp className="w-10 h-10 text-violet-400" />
              </div>
              <h1 className="text-3xl font-bold text-white mb-2 bg-gradient-to-r from-violet-400 via-fuchsia-400 to-pink-400 bg-clip-text text-transparent">
                Create Account
              </h1>
              <p className="text-gray-400">Join Stock Analyzer and start your investment journey</p>
            </div>
            ...
            {/* Form - Same interactivity as Login */}
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="group">
                ...
                <div className="relative overflow-hidden">
                  ...
                  <input
                    ...
                    className={`block w-full pl-12 pr-4 py-4 bg-black/30 backdrop-blur-sm border border-violet-400/20 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 focus:bg-black/40 transition-all duration-300 relative z-10 ${
                      errors.email 
              ...
              <div className="group">
                ...
                <div className="relative overflow-hidden">
                  ...
                  <input
                    ...
                    className={`block w-full pl-12 pr-12 py-4 bg-black/30 backdrop-blur-sm border border-violet-400/20 rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 focus:bg-black/40 transition-all duration-300 relative z-10 ${
                      errors.password 
                        ? 'border-red-500/50 bg-red-500/5' 
                        : ''
                    }`}
                ...
                <div className="relative overflow-hidden">
                  ...
                  {liquidFields.confirmPassword && (
                    <div className="absolute inset-0 pointer-events-none rounded-xl overflow-hidden">
                      <svg className="liquid-wave-svg" viewBox="0 0 400 100" preserveAspectRatio="none">
                        <defs>
                          <linearGradient id="liquidGradient3" x1="0%" y1="0%" x2="0%" y2="100%">
                            <stop offset="0%" stopColor="rgba(239, 68, 68, 0.8)" />
                            <stop offset="50%" stopColor="rgba(220, 38, 38, 0.6)" />
                            <stop offset="100%" stopColor="rgba(185, 28, 28, 0.4)" />
                          </linearGradient>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-violet-600 via-fuchsia-600 to-pink-600 text-white py-4 px-6 rounded-xl font-medium hover:from-violet-700 hover:via-fuchsia-700 hover:to-pink-700 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:ring-offset-2 focus:ring-offset-black disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-violet-500/25 flex items-center justify-center"
              >
                {loading ? (
                  <>
            ...
            <div className="mt-8 text-center">
              <p className="text-gray-400">
                ...
                <button
                  onClick={onToggleMode}
                  className="text-violet-400 hover:text-fuchsia-400 font-medium transition-colors hover:underline"
                >
                  Sign in here
                </button>
              </p>
